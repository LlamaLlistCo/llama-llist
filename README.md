...
│       ├── ets/
│       │   ├── pages/                        # 页面
│       │   │   ├── Index.ets                 # 笔记列表页（含平板双栏适配）
│       │   │   ├── NoteEdit.ets              # 新增/编辑笔记页（Markdown Native 预览/导入等）
│       │   │   ├── TagManage.ets             # 标签管理页
│       │   │   ├── TodoBoard.ets             # 待办看板页
...
│       │   │   └── native/
│       │   │       ├── KeywordNative.ets     # NAPI：关键词提取（Native）
│       │   │       └── MarkdownNative.ets    # NAPI：Markdown 解析/导出（Native）
│       │   ├── models/                       # 数据模型
│       │   │   ├── Note.ets                  # 笔记模型
│       │   │   ├── Tag.ets                   # 标签模型
│       │   │   └── Todo.ets                  # 待办模型
│       │   └── database/                     # 本地数据库（预留）
│       └── cpp/                              # Native 模块（C++，NAPI 暴露到 ArkTS）
│           ├── CMakeLists.txt
│           ├── keyword_napi.cpp              # NAPI 导出与模块注册
│           ├── keyword_napi.h                # Markdown AST / Native API 声明
│           ├── markdown_engine.cpp           # Markdown 解析引擎实现
│           └── markdown_engine.h
├── backend/                                  # Python 后端
│   ├── app/
│   │   ├── __init__.py
...
│   │       ├── files.py                      # 图片上传 API
│   │       ├── ai.py                         # AI 总结/搜索增强等 API
│   │       └── settings.py                   # 设置中心（含模型检测）
